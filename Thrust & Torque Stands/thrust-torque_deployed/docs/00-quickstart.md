...
open `http://localhost:8765`, or the Mac's VPN address from an iPad):
...
RPM setpoints: 1600, 1700, **1800**, 1900, 2000. 1800 is the flying point; run it first.
...
**About 446 W electrical per nacelle at 7.5 lbf and 1800 rpm, out of ground effect.**
That is Austin's flight-test figure, per nacelle, and it is 21 A at 21 V. Your OGE point at
1800 must land near it before anything else in the campaign means anything.

The BEMT model in this package predicts 274 W at that point. It is wrong by about 1.6 and
the torque stand (doc 14) is how you find out whether that is the rotor or the drivetrain.
Until then treat every predicted watt figure as a shape, not a value.

Hover collective per the model is about 16 deg at 0.75R at 1600 rpm falling to 10 deg at
2000; expect a few degrees more in reality. Ground effect saving at z/R = 0.5 will be real
but smaller than the 33 percent thrust benefit the textbook relation predicts, because
ground effect reduces induced power only.

Cruise after transition is 1000 rpm at 185 W per nacelle. That is forward flight and this
rig cannot reproduce it.
