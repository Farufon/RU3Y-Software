...
Size the meter for **50 A per nacelle**, not for the hover figure. Hover is **21 A at 21 V**
(446 W per nacelle from flight test; the 12 to 13 A in earlier versions of this document was
a model figure and was wrong), and peak thrust draw is roughly double that. A meter clipping
at peak loses you exactly the data point that matters for control authority.

The same 21 A applies to everything else in the power path. Pack leads, the loom to the
nacelle, and every connector it passes through must be rated for it with margin. In
particular, if you are running pack or phase current through a Micro-Fit 3.0, one circuit is
good for about 8.5 A with 18 AWG and derates further with all fourteen energised; that
connector is fine for signals and wrong for power unless several circuits are paralleled per
the derating chart.
