...
| Setpoint | Tip speed | BEMT hover W | Measured hover W | Predicted max thrust | Margin |
|---|---|---|---|---|---|
| 1600 | 76.6 m/s | 276 | | 8.6 lbf | 1.14 |
| 1700 | 81.4 m/s | 276 | | 9.7 lbf | 1.29 |
| **1800** | 86.1 m/s | 280 | **446** (flight) | 10.8 lbf | 1.44 |
| 1900 | 90.9 m/s | 286 | | 12.1 lbf | 1.61 |
| 2000 | 95.7 m/s | 294 | | 13.4 lbf | 1.78 |

**1800 is the flying point.** The 446 W column is Austin's flight-test figure per nacelle
and is the number your OGE point at 1800 rpm must land near before any other row means
anything. If it does not, stop and find out why before spending the campaign. The BEMT
column is the model, now known to be about 1.6 low at this point; keep it in the table so
the shape of the RPM trend can still be compared even though the level cannot.

The model says hover power is nearly flat across this range, so **do not expect a dramatic
optimum**. That is a statement about the shape of the curve and may survive the model being
1.6 low in level; the sweep is what tells you. If the measured spread across the RPM ladder is under about 5 percent, that is
...
**Current.** Hover is now 21 A per nacelle at 21 V, not the 13 A the earlier documents
assumed, and the thrust ceiling will pull well past that. Before the first full-power run
confirm the ESC rating, the pack lead and loom gauge, every connector in the power path,
and that the watt meter reads past 40 A without clipping. A Micro-Fit 3.0 circuit is good
for about 8.5 A; if pack or phase current runs through one, it needs several circuits
paralleled and the derating chart, not one pin.
