...
**Activate the virtual environment first.** This Mac has more than one Python
and the system one must not be installed into:

```
source ~/venvs/forcerigs/bin/activate
```

If it does not exist yet, build it per `docs/15-python-venv.md`.
...
(Still inside the environment. If you opened a new window, `source` it again.)
