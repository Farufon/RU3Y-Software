...
# Flight-test anchor, per nacelle (Austin Meyer, September 2026). The OGE
# point at 1800 rpm on the stand must land near this before the rest of the
# campaign means anything. Cruise (1000 rpm, 185 W) is forward flight and
# is not a static condition, so it is not a bench target.
MEASURED_HOVER_RPM = 1800
MEASURED_HOVER_W = 446.0
MEASURED_HOVER_LBF = 7.5

# Collective staircase, in DEGREES AT 0.75R, per governed RPM setpoint.
# BEMT hover pitch falls from ~16 deg at 1600 rpm to ~10 deg at 2000 rpm, so
# each ladder is shifted to bracket 7.5 lbf with points either side. The model
# runs about 1.6 low on power, so expect real hover a few degrees above these.
COLLECTIVE_STEPS = {
...
def action_calibrate(link):
    ...
    print("\n  Step 4: FLIP THE SIGN. This is required, not optional.")
    print("  You calibrated in compression (weight pressing down on the cell).")
    print("  Thrust pulls UP. Without this flip every reading is logged negative.")
    input("  Press Enter to send 'f'...")
    link.drain()
    link.send("f")
    time.sleep(1.0)
    while not link.lines.empty():
    ...
    print("\n  Step 5: confirm. Lift the nacelle mount by hand and hold it.")
    input("  Lifting? Press Enter...")
    s = summarise(collect(link, 3.0, show=False))
    if s:
        print(f"    reading while lifting: {s['mean']:+.4f} lbf")
        if s["mean"] > 0:
            print("    OK. Up reads positive. That is what you want.")
        else:
            print("    ! Up still reads NEGATIVE. Send 'f' again, or you lifted")
            print("      nothing. Do not start a campaign until this is positive.")

    print("\n  Step 6: linearity check. Apply each weight and record.")
...
def action_run(link, csv_path, raw_dir):
    ...
    def capture(coll_label, coll_value):
        ...
        watts = volts * amps
        print(f"    => {watts:.0f} W")
        if abs(rpm_cmd - MEASURED_HOVER_RPM) < 1 and abs(st['mean'] - MEASURED_HOVER_LBF) < 1.0:
            print(f"       flight-test hover at {MEASURED_HOVER_RPM} rpm is {MEASURED_HOVER_W:.0f} W;"
                  f" this point is {100*watts/MEASURED_HOVER_W:.0f}% of that")
        print()
