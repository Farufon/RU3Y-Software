...
const uint16_t STREAM_MS     = 100;   // 10 Hz output while streaming
...
void printHelp() {
  ...
  Serial.println(F("  s            start / stop streaming at 10 Hz"));
  Serial.println(F("  c <lbf>      calibrate: tare first, hang known weight, then e.g. 'c 5.0'"));
  Serial.println(F("  f            flip sign (use if thrust reads negative)"));
